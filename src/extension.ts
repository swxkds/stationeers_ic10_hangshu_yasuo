...
function 生成IC10映射表入栈指令(input: string): string {
  ...
  const 源物品指针数组 = 将物品哈希转换成物品槽指针(Array.from(全部源物品哈希表), {
    初始容量: 初始数组长度, 最大容量: 最大数组长度, 物品槽容量: 1
  });
  ...
  指令序列.push(`#<源物品指针数组>基址=1  -------  长度=${源物品指针数组.实际容量}`);
  ...
  for (const [哈希, { 指针地址: 指针, 物品表基址偏移: 偏移 }] of 源物品指针数组.物品表.entries()) {
...
function 生成配方入栈指令(input: string): string {
  ...
  const 物品特征 = /^#试剂物品=>\s+(\S+)\s+(\S+)\s+(\S+)/;
  const 配方特征 = /^#配方=>\s+(\S+)\s+(\S+)\s+成分=>\s+(.+)/;

  const 试剂简名映射试剂哈希 = new Map<string, number>();
  const 试剂哈希映射物品哈希 = new Map<number, number[]>();
  const 试剂哈希映射试剂详情 = new Map<number, [number, string]>();
  const 物品哈希映射物品详情 = new Map<number, [number, string, number]>();
  const 试剂哈希映射试剂编号 = new Map<number, number>();
  const 全部物品哈希表 = new Set<number>();
  const 配方详情 = new Map<number, [number, string, [string, string][]]>();

  const 原始代码 = input.split(/\r?\n/);

  let 当前父级试剂哈希: number | undefined;
  for (let rawLine of 原始代码) {
    const line = rawLine.trim();
    if (!line) { continue; }
    ...
    if (试剂) {
      ...
      试剂哈希映射物品哈希.set(哈希, []);
      试剂哈希映射试剂详情.set(哈希, [哈希, 简名]);
      当前父级试剂哈希 = 哈希;
      continue;
    }

    const 物品 = line.match(物品特征);
    if (物品) {
      const 名称 = 物品[1];
      const 简名 = 物品[2];
      const 储量 = 物品[3] || "null";
      ...
      if (当前父级试剂哈希 !== undefined) {
        const 列表 = 试剂哈希映射物品哈希.get(当前父级试剂哈希) || [];
        列表.push(哈希);
        试剂哈希映射物品哈希.set(当前父级试剂哈希, 列表);
      }

      物品哈希映射物品详情.set(哈希, [哈希, 简名, 解析储量(储量)]);
      全部物品哈希表.add(哈希);
      continue;
    }
    ...
    if (配方) {
      ...
      const 条目表 = 成分表_文本.split(/\s+/).map(s => s.trim()).filter(Boolean);

      for (const __ of 条目表) {
        const 分隔位置 = __.indexOf("X");
        if (分隔位置 <= 0) { continue; }
        const 名称 = __.substring(0, 分隔位置);
  ...
  // 把成分中的试剂名称转换成试剂哈希（优先使用试剂简名映射）
  for (const [哈希, 哈希_简名_成分表] of 配方详情) {
    const 成分表 = 哈希_简名_成分表[2];

    const 转换后成分: [string, string][] = 成分表.map(([名称, 数量]) => {
      if (试剂简名映射试剂哈希.has(名称)) {
        return [String(试剂简名映射试剂哈希.get(名称))!, 数量];
      }
      else {
        return [String(转换整数哈希(名称)), 数量];
      }
    });

    配方详情.set(哈希, [哈希_简名_成分表[0], 哈希_简名_成分表[1], 转换后成分]);
  }

  const 初始哈希桶数 = 0;
  const 堆栈尺寸 = 500;

  const 物品指针表 = 将物品哈希转换成物品槽指针(Array.from(全部物品哈希表), {
    初始容量: 初始哈希桶数, 最大容量: 堆栈尺寸, 物品槽容量: 3
  });

  if (!物品指针表) { return "ERROR: 无法为物品生成唯一快查索引（可能物品数量过多或哈希冲突无法解决）"; }
  ...
  const 物品指针表起始 = 1;
  const 物品指针表长度 = 物品指针表.实际容量;

  const 配方表起始 = 物品指针表.实际容量 + 2;
  const 配方数据头尺寸 = 3;
  const 最多合成次数 = 3;     // 试剂编号占8位,试剂数量占8位,共16位, 52位尾数/ 16 = 3.25
  const 配方成分表尺寸 = Math.ceil(最多试剂数 / 最多合成次数);
  const 配方表长度 = 配方表Arr.length * (配方数据头尺寸 + 配方成分表尺寸);

  const 试剂数据头尺寸 = 4;
  const 试剂表起始 = 配方表起始 + 配方表长度 + 2;
  const 试剂表长度 = (试剂简名映射试剂哈希.size) * (试剂数据头尺寸 + 最多物品数);

  const 物品表起始 = 试剂表起始 + 试剂表长度 + 2;
  const 物品表长度 = (全部物品哈希表.size) * 3;

  // --- 生成指令 ---
  const 指令序列: string[] = [];
  指令序列.push(`#<物品指针表>基址=${物品指针表起始}  -------  长度=${物品指针表长度}`);
  指令序列.push(`#<配方表>基址=${配方表起始}  -------  长度=${配方表长度}`);
  指令序列.push(`#<试剂表>基址=${试剂表起始}  -------  长度:${试剂表长度}`);
  指令序列.push(`#<物品表>基址=${物品表起始}  -------  长度:${物品表长度}`);
  指令序列.push(`#总堆栈占用:${物品表起始 + 物品表长度 - 1}`);

  指令序列.push(``);
  指令序列.push(`#通过槽位的物品,读取其哈希,通过<(Math.abs(物品哈希) % <物品指针表>长度) + 1>运算后得到物品指针`);
  指令序列.push(`#再用get[指针]获取物品槽地址, pop 物品哈希 => 用于发生哈希碰撞时跳过写入, pop 物品储量 => 储量加减, pop 试剂指针 => 统计总和数量加减`);
  指令序列.push(``);
  ...
  // 1️⃣ 物品指针表
  for (const [物品哈希, { 指针地址, 物品表基址偏移 }] of 物品指针表.物品表.entries()) {
    const 简名 = 物品哈希映射物品详情.get(物品哈希)?.[1] || "未知";
    指令序列.push(`poke ${指针地址 - 1} ${物品表基址偏移 + 物品表起始 + 2}      #物品表[${简名}, ${物品哈希}]`);
  }
  指令序列.push(``);

  // 为试剂分配编号（用于生成十六进制标签A,B...）
  let 编号计数 = 0;
  for (const [_简名, 试剂哈希] of 试剂简名映射试剂哈希) {
    编号计数++;
    试剂哈希映射试剂编号.set(试剂哈希, 编号计数);
  }
  ...
  // 2️⃣ 配方表 写入（从高地址向下写）
  let 当前配方写入地址 = 配方表起始 + 配方表长度 - 1;
  for (const [配方哈希, [_, 配方简名, 成分表]] of 配方表Arr) {
    const 最小生产数量 = 成分表.reduce((sum, [_, 数量]) => sum + parseInt(数量, 10), 0);
    const 试剂成分数 = 成分表.length;

    const 十六进制成分表数组: string[] = [];
    let 成分注释 = '';

    for (const [试剂名称, 数量] of 成分表) {
      const 试剂哈希 = 试剂简名映射试剂哈希.get(试剂名称) || 转换整数哈希(试剂名称);
      const 编号 = 试剂哈希映射试剂编号.get(试剂哈希) || 0;
      const 条目 = `${编号.toString(16).toUpperCase().padStart(2, '0')}_${parseInt(数量, 10).toString(16).padStart(2, '0')}`;
      十六进制成分表数组.push(条目);

      const 注释名 = 试剂哈希映射试剂详情.get(试剂哈希)?.[1] || String(试剂哈希);
      成分注释 += `${注释名}X${数量} `;
    }
    ...
    const 成分表拆分 = splitArray(十六进制成分表数组, 最多合成次数);
    const 十六进制成分表列表 = 成分表拆分.map(group => group.join("__"));

    let 计数 = 当前配方写入地址;
    计数--;
    指令序列.push(`poke ${计数} ${配方哈希} #配方[${配方简名}, ${配方哈希}]`);
    计数--;
    指令序列.push(`poke ${计数} ${最小生产数量} #最小生产数量`);
    计数--;
    指令序列.push(`poke ${计数} ${试剂成分数} #试剂成分数`);

    指令序列.push(`#${成分注释.trim()}`);
    for (const __ of 十六进制成分表列表) {
      计数--;
      指令序列.push(`poke ${计数} $${__} #成分表`);
    }

    指令序列.push(``);
    当前配方写入地址 -= (配方数据头尺寸 + 配方成分表尺寸);
  }

  const 试剂地址表 = new Map<number, number>();

  指令序列.push(`\n\n\n`);
  指令序列.push(`#试剂表`);

  // 3️⃣ 试剂表
  let 当前试剂写入地址 = 试剂表起始 + 试剂表长度 - 1;

  指令序列.push(`#通过${当前试剂写入地址 + (试剂数据头尺寸 + 最多物品数)} - 试剂编号 * ${(试剂数据头尺寸 + 最多物品数)} 定位到试剂地址`);
  指令序列.push(``);

  let 试剂编号计数 = 0;
  for (const [试剂简名, 试剂哈希] of 试剂简名映射试剂哈希) {
    const 所属物品表 = 试剂哈希映射物品哈希.get(试剂哈希) || [];
    const 同类物品数 = 所属物品表.length;

    试剂地址表.set(试剂哈希, 当前试剂写入地址);

    let 计数 = 当前试剂写入地址;
    计数--;
    指令序列.push(`poke ${计数} ${试剂哈希}     #试剂表[${试剂简名}, ${试剂哈希}, 试剂编号(${++试剂编号计数})]`);
    计数--;
    指令序列.push(`poke ${计数} 0     #请求提货数量`);
    计数--;
    指令序列.push(`poke ${计数} 0     #统计总和数量`);
    计数--;
    指令序列.push(`poke ${计数} ${同类物品数}     #同类物品数`);

    所属物品表.forEach((物品哈希) => {
      const 物品表地址 = 物品指针表.物品表.get(物品哈希)!.物品表基址偏移 + 物品表起始 + 2;
      const 简名 = 物品哈希映射物品详情.get(物品哈希)?.[1] || "未知";
      计数--;
      指令序列.push(`poke ${计数} ${物品表地址}      #物品表[${简名}, ${物品哈希}]`);
    });

    指令序列.push(``);
    当前试剂写入地址 -= (试剂数据头尺寸 + 最多物品数);
  }
  ...
  指令序列.push(``);
  // 4️⃣ 物品表
  for (const [物品哈希, [_, 简名, 储量]] of 物品哈希映射物品详情) {
    const 物品表地址 = 物品指针表.物品表.get(物品哈希)!.物品表基址偏移 + 物品表起始 + 2;
    let 计数 = 物品表地址;

    const 对应试剂表: number[] = [];
    for (const [试剂哈希, 试剂物品表] of 试剂哈希映射物品哈希) {
      if (试剂物品表.includes(物品哈希)) {
        对应试剂表.push(试剂哈希);
      }
    }
    const 对应试剂 = 对应试剂表.length ? 对应试剂表[0] : 0;

    计数--;
    指令序列.push(`poke ${计数} ${物品哈希}     #物品表[${简名}, ${物品哈希}]`);
    计数--;
    指令序列.push(`poke ${计数} ${储量}         #储量`);
    计数--;
    指令序列.push(`poke ${计数} ${试剂地址表.get(对应试剂) || 0}     #试剂表[${试剂哈希映射试剂详情.get(对应试剂)?.[1] || ''}]`);
    指令序列.push(``);
  }
...
function 将物品哈希转换成物品槽指针(all: number[], __: { 初始容量: number, 最大容量: number, 物品槽容量: number }) {

  let 哈希桶容量 = Math.max(Math.ceil(all.length), __.初始容量);
  if (哈希桶容量 < 1) { 哈希桶容量 = 1; }

  while (哈希桶容量 <= __.最大容量) {

    const 防重 = new Set<number>();
    const 物品表 = new Map<number, { 指针地址: number, 物品表基址偏移: number }>();
    ...
    for (const intHash of all) {
      ...
      物品表.set(intHash, { 指针地址: 取指针地址, 物品表基址偏移: 计数 * __.物品槽容量 });
      计数++;

    }

    if (!哈希碰撞么) { return { 实际容量: 哈希桶容量, 物品表: 物品表 }; }
    哈希桶容量++;
  }
