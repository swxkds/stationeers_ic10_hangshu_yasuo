...
function 生成配方入栈指令(input) {
    ...
    const 试剂哈希映射试剂物品哈希 = new Map();
    const 试剂物品哈希映射试剂物品详情 = new Map();
    const 全部试剂物品哈希表 = new Set();
    const 配方详情 = new Map();
    let 当前父级试剂哈希;
    for (let rawLine of 原始代码) {
        const line = rawLine.trim();
    ...
    for (const [哈希, 哈希_简名_成分表] of 配方详情) {
        ...
        const __ = 成分表.map(([名称, 数量]) => {
            if (试剂简名映射试剂哈希.has(名称)) {
                {
                    return [String(试剂简名映射试剂哈希.get(名称)), 数量];
                }
            } // 如果源码中的成分名称是试剂简名   // !是TypeScript中的非空断言操作符
            else {
                return [String(转换整数哈希(名称)), 数量];
            } // 如果源码中的成分名称是 HASH("Name") 或 Name 或 整数哈希 这三种
        });
            ...
            if (e < 成分表.length) {
                ...
                for (let k = 0; k < 最大试剂物品数; k++) {
                    if (k < items.length) {
                        ...
                        if (__) {
                            指令序列.push(`poke ${高位首地址 - 计数} ${试剂物品统计槽表起始 + __.统计槽表基址偏移}          #试剂物品统计槽[${简名} , ${试剂物品哈希}]`);
                        }
                        else {
                            指令序列.push(`poke ${高位首地址 - 计数} 0            #该试剂无其它可代替物品`);
                        }
                    }
                    else {
                        计数++;
    ...
    for (const [哈希, { 指针地址: 指针, 统计槽表基址偏移: 偏移 }] of 试剂物品统计槽指针数组.统计槽表.entries()) {
        ...
        for (let k = 2; k < 试剂物品统计槽长度; k++) {
            计数++;
            指令序列.push(`poke ${____ - 计数} 0                              #占位`);
        }
    }
    指令序列.push("");
    指令序列.push(`#写入完毕`);
...
function 将试剂物品哈希映射到试剂物品统计槽堆栈指针(all, options) {
