...
function 生成IC10映射表入栈指令(input) {
    const 源特征 = /^#源物品=>\s+(\S+)\s+(\S+)/;
    const 目标特征 = /^#目标物品=>\s+(\S+)\s+(\S+)/;
    const 简名映射哈希 = new Map();
    const 源物品映射目标物品 = new Map();
    const 物品哈希映射物品详情 = new Map();
    const 全部源物品哈希表 = new Set();
    ...
    if (!源物品指针数组) {
        return "ERROR: 无法为物品生成唯一快查索引（可能物品数量过多或哈希冲突无法解决）";
    }
    // --- 开始生成指令列表
    const 指令序列 = [];
...
function 生成配方入栈指令(input) {
    const 试剂特征 = /^#试剂=>\s+(\S+)\s+(\S+)/;
    const 物品特征 = /^#试剂物品=>\s+(\S+)\s+(\S+)\s+(\S+)/;
    const 配方特征 = /^#配方=>\s+(\S+)\s+(\S+)\s+成分=>\s+(.+)/;
    const 试剂简名映射试剂哈希 = new Map();
    const 试剂哈希映射物品哈希 = new Map();
    const 试剂哈希映射试剂详情 = new Map();
    const 物品哈希映射物品详情 = new Map();
    const 试剂哈希映射试剂编号 = new Map();
    const 全部物品哈希表 = new Set();
    const 配方详情 = new Map();
    const 原始代码 = input.split(/\r?\n/);
    let 当前父级试剂哈希;
    for (let rawLine of 原始代码) {
...
function 转换整数哈希(str) {
    ...
    if (match) {
        return (crc32Signed(match[1]));
    } // HASH("Name") -> 计算 crc32Signed(Name)
    if (/^-?\d+$/.test(string)) {
